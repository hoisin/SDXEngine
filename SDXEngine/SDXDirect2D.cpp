...
{
	if (pDX == nullptr)
		return SDXErrorId::SDX_ERROR_DIRECT2D_NULLPTR_SET;
...
{
	if (!m_bInit)
		return SDXErrorId::SDX_ERROR_DIRECT2D_NOT_INITIALISED;

	if (m_context == nullptr)
		return SDXErrorId::SDX_ERROR_DIRECT2D_DEVICE_NOT_CREATED;

	// Default brush colour yellow, expand for other colours later
	if (m_textFormat == nullptr || m_colorBrush == nullptr)
		return SDXErrorId::SDX_ERROR_DIRECT2D_TEXTFORMAT_NOT_SETUP;
	...
	return SDXErrorId::SDX_ERROR_DIRECT2D_DRAWTEXT_FAILED;
}
...
SDXErrorId SDXEngine::SDXDirect2D::SetRenderText(const std::string text)
{
	...
	return SDXErrorId::SDX_ERROR_DIRECT2D_SETUPTEXTFORMAT_FAILED;
}
...
SDXErrorId SDXEngine::SDXDirect2D::CreateDevice()
{
	if (m_directX == nullptr)
		return SDXErrorId::SDX_ERROR_DIRECT2D_NOT_SET_DIRECTX;
	...
	if (m_device || m_context)
		return SDXErrorId::SDX_ERROR_DIRECT2D_DEVICE_ALREADY_CREATED;
	...
	if (FAILED(result))
		return SDXErrorId::SDX_ERROR_DIRECT2D_DEVICE_CREATE_FAILED;
...
SDXErrorId SDXEngine::SDXDirect2D::CreateBitmapRenderTarget()
{
	...
	return SDXErrorId::SDX_ERROR_DIRECT2D_BITMAPRENDERTARGET_CREATE_FAILED;
}
...
SDXErrorId SDXEngine::SDXDirect2D::InitialiseTextFormats()
{
	...
	if (FAILED(result))
		return SDXErrorId::SDX_ERROR_DIRECT2D_CREATEBURSHES_FAILED;
