...
TEST_F(SDXDirectXShaderCompilerUTest, Compile)
{
	...
	EXPECT_EQ(error, SDXErrorId::SDX_ERROR_DXCOMPILER_COMPILE_INVALID_ARGS) << "Expected fail on invalid args";
	...
	EXPECT_EQ(error, SDXErrorId::SDX_ERROR_NONE) << "Valid vertex shader compile fail";
	if (testShader)
	{
		testShader->Release();
		testShader = nullptr;
	...
	EXPECT_EQ(error, SDXErrorId::SDX_ERROR_NONE) << "Valid pixel shader compile fail";
	if (testShader)
	{
		testShader->Release();
		testShader = nullptr;
	...
	EXPECT_EQ(error, SDXErrorId::SDX_ERROR_DXCOMPILER_COMPILE_FAILED) << "Expected error on invalid vertex shader compile";
	if (testShader)
	{
		testShader->Release();
		testShader = nullptr;
